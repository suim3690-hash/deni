...
응답은 `{"eventId":"...","hazardId":"... 또는 null"}`이다. 같은 ID로 다른 원본을 보내면 `409`다. 최초 수신 시각은 DB가 기록한다. 같은 프레임에 여러 물체가 있으면 사진은 같아도 **물체마다 다른 eventId**를 보낸다. 지원 라벨이면 백엔드가 아이의 현재 프로필로 위험을 생성·갱신한다.
