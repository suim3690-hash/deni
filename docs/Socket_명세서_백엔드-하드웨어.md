...
수동 이송의 `taskState`는 `ALIGNING_TARGET → CAPTURING → SEEKING_MARKER → PUSHING_TO_MARKER → BACKING → VERIFYING_DROP → TURNING_AROUND` 순서다. 마커가 보이지 않을 때는 전진하지 않으며, 배치 확인 실패 결과에는 `relocationCompleted`를 넣지 않는다.
