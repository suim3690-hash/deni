...
> **2026-09-21 프론트 갱신:** 전원 버튼은 연결된 뒤에도 다시 눌러 끌 수 있다. 목업 화면은 즉시 꺼진 상태로 바뀌고,
> 실제 API 모드는 백엔드에 전원 끄기 명령이 없어 "아직 지원되지 않아요" 안내만 표시하며 서버 상태를 바꾸지 않는다.
> 홈은 `robot-state`의 `operationState`·`movementState`를 작동 중·일시 정지·이송 중 / 전진·회전·후진·정지 배지로 표시한다.
> 위험 상세의 `location.marker`가 있으면 스마트 안심 케어 맵에 그 좌표로 마커를 표시하고, 없으면 마커를 생략한다(오류 문구 없음).
> 지도 화면이 열려 있는 동안 위험 상세를 5초마다 다시 조회해 좌표가 바뀌면 마커가 부드럽게 이동한다.
> 안전 이송 요청 후 프론트는 `GET /safety-actions/{actionId}`를 3초마다 조회하고, `status` 또는 `treatmentStatus`가
> `TEMPORARY_COMPLETED`일 때만 이송 완료로 표시한다. `FAILED`면 직접 제거 안내를 표시한다.
> 감지 사진이 없거나 불러오지 못하면 실제 모드는 "감지 사진을 불러오지 못했어요"를 표시하고, 목업 사진은 목업 화면에서만 쓴다.
...
| `위험 물체 제거 완료` | `POST /hazards/{hazardId}/removal-checks` | 요청 식별키 | 처리 ID, `CHECKING` | 로봇 정지 유지·`재확인 중` 표시. 최종 결과 조회 |
| `위험 물체 안전 이송` | `POST /hazards/{hazardId}/relocations` | 요청 식별키, 필요 시 안전 위치 ID | 처리 ID, `MOVING` | `이동 중` 표시. 실제 이동 완료 후에만 임시 완료 표시 |
...
보호자가 실제 위험물을 치운 뒤 `위험 물체 제거 완료`를 누를 때만 호출한다. 본문 `{}`, `Idempotency-Key` 필수. `202 Accepted`:
...
보호자가 `위험 물체 안전 이송`을 선택할 때 호출한다. 안전 위치를 서버에 미리 지정한다면 본문 `{}`, 사용자가 선택한다면 `{ "safeZoneId": "zone_123" }`를 사용한다. `Idempotency-Key` 필수. `202 Accepted`:
...
| 스마트 안심 케어 맵 `사용자 직접 제거` | 제거 안내 화면으로 이동만 수행. 실제 API 요청은 `위험 물체 제거 완료`에서 수행 |
...
| `위험 구역 진입시 자동 이송 모드` 토글 | 보호자 선택 없이 이동하는 규칙·대상 물체·실패 처리 미정. 이번 핵심 API에서 제외. 누르면 "아직 지원되지 않아요" 안내만 표시(코드 기준) |
...
4. ~~안전 위치 이동 후 청소를 자동 재개할지, 보호자에게 별도의 `청소 재개` 버튼을 제공할지.~~ **[2026-09-21 프론트 결정]** 이동 완료 후 자동 재개하지 않고, `위험물을 처리했습니다` 안내와 함께 별도 `청소 재개` 버튼을 제공한다. 버튼은 `POST /devices/{deviceId}/commands/resume`을 호출하며, 백엔드가 재개를 접수하기 전에는 서버 오류 메시지를 그대로 안내한다.
