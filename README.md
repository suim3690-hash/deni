...
현재 연결 구조는 `프론트 → HTTP → 백엔드 ↔ WebSocket → 하드웨어`이며, 탐지 이미지는 하드웨어가 백엔드로 HTTP 업로드한다. 백엔드가 PostgreSQL에 요청과 결과를 저장한다. 전원·정지·직접 제거 재확인·수동 이송 경로가 연결돼 있고 자동 이송·실측 지도 좌표는 미구현이다. 수동 이송은 물체를 반원형 팔에 확보한 뒤 마커를 따라 밀고, 후진 후 물체가 마커 주변에 놓였는지 재확인해야 완료된다. 생활공간 위험은 사용자가 확인 완료하면 DB에서 `RESOLVED`로 처리되어 알림과 활성 지도 마커에서 제거된다. 같은 라벨이 계속 보이면 완료 상태를 유지하고 30초 이상 미검출 뒤 다시 나타날 때 새 위험으로 등록한다. 실행 예제와 지원 범위는 [통합 현황](docs/현황_2026-09-22_통합연결.md)을 따른다.
