...
현재 연결 구조는 `프론트 → HTTP → 백엔드 ↔ WebSocket → 하드웨어`이며, 탐지 이미지는 하드웨어가 백엔드로 HTTP 업로드한다. 백엔드가 PostgreSQL에 요청과 결과를 저장한다. 전원·정지·직접 제거 재확인·수동 이송 경로가 연결돼 있고 자동 이송·실측 지도 좌표는 미구현이다. 실행 예제와 지원 범위는 [통합 현황](docs/현황_2026-09-22_통합연결.md)을 따른다.
