...
요청과 결과를 저장한다. 현재 하드웨어 WebSocket은 설계 단계이고 탐지·상태는 공유 DB에 직접 저장할 수 있다.
...
| `v2/frontend` | 필요할 때 만드는 프론트 작업 브랜치 |
| `v2/backend` | 필요할 때 만드는 백엔드 작업 브랜치 |
| `v2/hardware` | 필요할 때 만드는 하드웨어 작업 브랜치 |
...
공유 DB에 직접 데이터를 넣는 현재 개발 방식은
[모델·하드웨어 DB 저장 가이드](docs/모델_하드웨어_DB_저장_가이드.md)를 참고한다.
