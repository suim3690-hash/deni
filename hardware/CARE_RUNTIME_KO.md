...
- 정지 상태에서 프론트가 두 가지를 요청할 수 있다.
  - **직접 제거 재확인**(`RECHECK_HAZARD`): 유효한 영상에서 대상이 2초 연속 보이지 않으면 완료로 보고한다. 완료 직후 해당 라벨의 중복 알림 쿨다운을 초기화하므로 다시 보이면 새 탐지 이벤트를 즉시 올린다.
...
- 조향은 오차 크기에 따라 나뉜다. `coarse_bearing` 밖에서는 멈추지 않고 연속 회전해 움직임이 끊기지 않고,
  그 안쪽에서는 짧은 펄스와 정지로 과회전을 막는다.
- 마커가 안 보이면 먼저 멈춰 한 번 더 보고, 그래도 없으면 **마지막으로 본 쪽으로 약 90° 훑은 뒤 반대로 돌아 한 바퀴를 채운다**.
  각 펄스 뒤에는 정지한 뒤 찍힌 프레임을 확인하고 나서야 다음 회전을 한다. 실패는 경과 시간이 아니라
  누적 회전량(`search_turns` 바퀴)으로 판단하며, 전체 제한은 `action_timeout_seconds`다.
- 마커를 찾고 미는 동안(`SEEKING_MARKER`, `PUSHING_TO_MARKER`)은 Pi 카메라 초점 범위를 `--marker-focus-range`(기본 `normal`)로,
  그 외에는 `--focus-range`(기본 `macro`)로 바꾼다. Pi 서버가 `POST /focus`를 지원하지 않으면 경고만 남기고 주행은 계속한다.
...
| `full_turn_seconds` | 6.0 | 제자리 한 바퀴에 걸리는 회전 시간 추정값. 엔코더가 없어 탐색 각도를 이 값으로 환산한다. **실측 보정 필요.** |
