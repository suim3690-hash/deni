...
export default function HazardLocation({ hazard, detail, error, errorStatus, isMock, onBack, onRetry }: Props) {
  ...
  function toggleAutoTransport() {
    if (!isMock) {
      setActionMessage('자동 이송 모드는 아직 지원되지 않아요. 안전 위치로 이동 또는 직접 제거를 이용해 주세요.')
      return
    }
  ...
  async function startManualTransport() {
    if (isMock) {
    ...
    try {
      ...
      setActionMessage(apiErrorMessage(err, '안전 위치 이동 요청을 접수하지 못했어요.'))
    }
  }
  ...
  async function startRemove() {
    ...
    try {
      ...
      setActionMessage(apiErrorMessage(err, '직접 제거 재확인 요청을 접수하지 못했어요.'))
    }
  }
      ...
      <div className="mx-auto min-h-screen max-w-[402px] pb-[112px]">
        ...
        {restricted ? <main className="px-4 pt-6"><div role="alert" className="rounded-[16px] border border-[#f2c5cb] bg-white p-5 text-[14px] text-[#9d1237]">{error}</div></main> : <main className="space-y-[17px] px-4 pt-3">
          ...
          <section aria-label="스마트 안심 케어 맵" className="rounded-[22px] bg-white p-4 shadow-[0_2px_8px_rgba(48,60,90,0.06)]">
            ...
            {!isMock && !detail && !error ? <p className="rounded-[16px] bg-[#f3f6fc] p-5 text-center text-[13px] text-[#64748b]">지도를 불러오고 있어요.</p> : (
              <>
          ...
          <section aria-label="위험물 감지 상세" className="rounded-[22px] bg-white px-[18px] pb-[18px] pt-[16px] shadow-[0_2px_8px_rgba(48,60,90,0.06)]">
            ...
            <p className="mt-2 text-center text-[11px] text-[#94a3b8]">프라이버시 보호를 위해 위험물만 확대해서 보여드려요</p>
        ...
        <footer className="fixed bottom-0 left-1/2 z-10 flex w-full max-w-[402px] -translate-x-1/2 gap-3 border-t border-[#e2e8f0] bg-white px-3 pb-5 pt-3">
          {isMock ? (
            autoTransport ? (
              ...
              <>
                <button type="button" onClick={() => void startManualTransport()} className="min-h-[54px] flex-1 rounded-[20px] border border-[#3755ff] bg-white px-2 text-[12px] font-semibold text-[#2948dd]">안전 위치로 이동</button>
                <button type="button" onClick={() => void startRemove()} className="min-h-[54px] flex-1 rounded-[20px] bg-[#b9003d] px-2 text-[13px] font-bold leading-4 text-white shadow-[0_3px_8px_rgba(185,0,61,0.25)]">사용자 직접 제거<br />(위험물 우회 청소)</button>
              </>
            )
          ) : relocationState === 'submitting' ? (
            <button type="button" disabled className="flex min-h-[54px] flex-1 items-center justify-center gap-2 rounded-[20px] bg-[#e5e7eb] px-2 text-[14px] font-semibold text-[#6b7280] disabled:cursor-default">
              <Loader2 size={16} className="animate-spin" aria-hidden="true" />이동 요청 접수 중
            </button>
          ) : relocationState === 'pending' ? (
            <div role="status" className="flex min-h-[54px] flex-1 flex-col items-center justify-center rounded-[20px] bg-[#eef2ff] px-2 text-center text-[#3730a3]">
              <strong className="text-[13px] font-bold">이동 요청이 접수됐어요</strong>
              <span className="text-[11px]">처리 결과는 아직 확인되지 않아요. 위험물이 보이면 직접 치워 주세요.</span>
            </div>
          ) : removalState === 'submitting' ? (
            <button type="button" disabled className="flex min-h-[54px] flex-1 items-center justify-center gap-2 rounded-[20px] bg-[#e5e7eb] px-2 text-[14px] font-semibold text-[#6b7280] disabled:cursor-default">
              <Loader2 size={16} className="animate-spin" aria-hidden="true" />재확인 요청 접수 중
            </button>
          ) : removalState === 'pending' ? (
            <div role="status" className="flex min-h-[54px] flex-1 flex-col items-center justify-center rounded-[20px] bg-[#eef2ff] px-2 text-center text-[#3730a3]">
              <strong className="text-[13px] font-bold">재확인 요청이 접수됐어요</strong>
              <span className="text-[11px]">기기가 위험물 부재를 확인하면 청소가 재개돼요. 아직 처리 결과는 확인할 수 없어요.</span>
            </div>
          ) : (
            <>
              <button type="button" onClick={() => void startManualTransport()} className="min-h-[54px] flex-1 rounded-[20px] border border-[#3755ff] bg-white px-2 text-[12px] font-semibold text-[#2948dd]">안전 위치로 이동</button>
