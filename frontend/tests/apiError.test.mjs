...
test('distinguishes reused registration requests and connection failures', () => {
  ...
  assert.match(apiErrorMessage(conflict, '기본 안내'), /이전 요청/)
...
test('reused operation keys do not incorrectly describe a child registration', () => {
  const conflict = new ApiRequestError(409, 'operation conflict', 'IDEMPOTENCY_KEY_REUSED', 'req-operation', {})
  const message = apiErrorMessage(conflict, '기본 안내')
  assert.match(message, /요청 식별키/)
  assert.match(message, /req-operation/)
  assert.doesNotMatch(message, /등록 요청/)
})
